import type { GenderRole, Message } from '../../../domain/types.js';
import type { LLMClient } from '../../../infra/adapters/llm.js';
import { ModelPreferenceAnalyzer, type LLMPreferenceSignal } from './model-preference-analyzer.js';
...
export class PreferenceDetector {
  private modelAnalyzer?: ModelPreferenceAnalyzer;

  constructor(llmClient?: LLMClient) {
    if (llmClient) {
      this.modelAnalyzer = new ModelPreferenceAnalyzer(llmClient);
    }
  }

  /**
   * Hybrid detect: rules first (0ms), LLM fallback for unmatched messages (~200ms).
   */
  async detectHybrid(message: string, recentContext?: Message[]): Promise<PreferenceSignal | LLMPreferenceSignal> {
    const ruleResult = detectByRules(message);
    if (ruleResult) return ruleResult;

    if (this.modelAnalyzer) {
      return this.modelAnalyzer.analyze(message, recentContext);
    }
  ...
  /**
   * Sync rule-only detect (backward compatible, no LLM call).
   */
  detect(message: string): PreferenceSignal {
    return detectByRules(message) ?? { type: 'none', confidence: 0, value: {}, source: 'conversation' };
  }
}
